...
const pngFixture = path.join(fixtureDir, 'png_opaque_64x32.png');
...
test.describe('Photo Studio ingress audit', () => {
  ...
  test('picker accepts the committed PNG through the shared import pipeline @portable', async ({ page }) => {
    ...
    await page.getByText('Import image', { exact: true }).evaluate((element) => (element as HTMLElement).click());
    const chooser = await chooserPromise;
    await chooser.setFiles(pngFixture);
    await expect(page.getByText('Replace image', { exact: true })).toBeVisible({ timeout: 15_000 });
  });
