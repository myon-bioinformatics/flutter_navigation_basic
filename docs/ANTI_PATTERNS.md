...
| `GENERATED_CODE_AS_SOURCE` | cross-cutting candidate | Dart/Markdown/TS generated from JSON is hand-edited or accumulates business logic | JSON/schema is authoritative; generated files are deterministic projections with headers and drift checks | review + future generator `--check` |
...
| `GOLDEN_WITHOUT_PROVENANCE` | cross-cutting candidate | Expected JSON is copied from the implementation under test and merely freezes its bug | Golden metadata names spec/known-value/provenance; independently derive computational expectations | fixture review/oracle tests |
